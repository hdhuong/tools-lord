...
# Function to check if a merge request already exists
check_existing_merge_request() {
  local project_id=$1
  ...
  merge_request_url=$(echo "${response}" | jq -r '.[0].web_url')

  if [ "${merge_request_url}" != "null" ]; then
    echo "An existing merge request already exists:"
...
create_merge_request() {
  ...
  if check_existing_merge_request "${project_id}" "${source_branch}" "${target_branch}"; then
    return
  fi
  ...
  if [ "${http_status}" -ne 201 ]; then
    ...
    merge_request_url=$(echo "${body}" | jq -r '.web_url')
...
select_projects() {
  echo "Select projects to create merge requests (comma-separated, e.g., 1,2,3):"
  echo "1) COST CENTER"
  echo "2) FILE WATCHER"
  echo "3) INVOICE"
  echo "4) ITEM PACKING"
  ...
  IFS=',' read -ra selected_projects <<< "$choices"

  for choice in "${selected_projects[@]}"; do
    case $choice in
      1)
